...
const Slide = ({ data, currentIndex, viewWidth }: SlideProps) => {
  ...
  return (
     <SlideWrap currentIndex={currentIndex} viewWidth={viewWidth}>
        {data && data.map((movie:MovieCard,index:number) => (
          movie && (
             <PostCard
              key={index}
...
const SlideWrap = styled.ul<{currentIndex:number, viewWidth:number}>`
  ...
  padding: 0 20px;
  width: 450vw;
  left: ${({ currentIndex, viewWidth}) => `-${currentIndex * Math.floor(viewWidth/ 7.5)}px`};
  transition: .3s;
  li{
    margin-right:15px;
  }
  @media ${({ theme }) => theme.deviceSize.max.mobile} {
    left: ${({ currentIndex, viewWidth}) => `-${currentIndex * Math.floor(viewWidth/ 4.4)}px`};
    li{
      margin-right:10px;
    }
  }
  ...
  @media (max-width: 500px) {
    left: ${({ currentIndex, viewWidth}) => `-${currentIndex * Math.floor(viewWidth/ 2.3)}px`};
  }
`