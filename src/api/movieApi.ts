...
export const getMovieByLatest = () => {
  const response = useHttpRequest('?_sort=date_uploaded&_order=desc&_limit=10');
  return response;
};

export const getMovieByRanking = () => {
  const response = useHttpRequest('?_sort=rating&_order=desc&_limit=10');
  return response;
};
